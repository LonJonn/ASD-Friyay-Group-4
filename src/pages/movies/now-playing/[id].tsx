...
async function getMoviesResult(search: string): Promise<GetNowPlayingMoviesResponse> {
    // Data is retrieved from the API layer
    const res = await fetch("/api/movies/now-playing/&" + search);
...
const MoviesSearchPage: NextPage = () => {
    ...
    const moviesQuery = useQuery<GetNowPlayingMoviesResponse, Error>({
        queryKey: ["nowPlaying", id],
    ...
    return (
        <Stack spacing={8}>
            ...
            <Heading size="sm">Date Range: {moviesQuery.data.startDate} - {moviesQuery.data.endDate}</Heading>
            ...
            {!moviesQuery.data.isLast ?
                <HStack space="24px">
                    ...
                    {moviesQuery.data.page != 1 ?
                        <NavigationButton search={String("")} nextPage={Number(moviesQuery.data.page - 1)} navigationDirection="Previous" endpoint="now-playing"></NavigationButton>
                    : "N/A"}
                    {/* The next button will show provided the outer if statement holds true */}
                    <NavigationButton search={String("")} nextPage={Number(moviesQuery.data.page + 1)} navigationDirection="Next" endpoint="now-playing"></NavigationButton>
                </HStack>
            :   
            ...
            {moviesQuery.data.isLast && moviesQuery.data.totalPages > 1 ? <NavigationButton search={String("")} nextPage={Number(moviesQuery.data.page - 1)} navigationDirection="Previous" endpoint="now-playing"></NavigationButton> : "N/A"}
        </Stack>
    );
};
