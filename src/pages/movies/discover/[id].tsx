...
async function getDiscoverResult(search: string): Promise<GetMoviesSearchResponse> {
    // Data is retrieved from the API layer
    const res = await fetch("/api/movies/discover/" + search);
...
const MoviesSearchPage: NextPage = () => {
    ...
    const moviesQuery = useQuery<GetMoviesSearchResponse, Error>({
        ...
        queryFn: () => getDiscoverResult(id as string),
    });
    ...
    return (
        <Stack spacing={8}>
            <Heading size="s">Discover results for: {router.query['id']}</Heading>
            ...
            {!moviesQuery.data.isLast ?
                <HStack space="24px">
                    ...
                    {moviesQuery.data.page != 1 ?
                        <NavigationButton search={String(router.query['id'])} nextPage={Number(moviesQuery.data.page - 1)} navigationDirection="Previous" endpoint="discover"></NavigationButton>
                    : "N/A"}
                    {/* The next button will show provided the outer if statement holds true */}
                    <NavigationButton search={String(router.query['id'])} nextPage={Number(moviesQuery.data.page + 1)} navigationDirection="Next" endpoint="discover"></NavigationButton>
                </HStack>
            :   
            ...
            {moviesQuery.data.isLast && moviesQuery.data.totalPages > 1 ? <NavigationButton search={String(router.query['id'])} nextPage={Number(moviesQuery.data.page - 1)} navigationDirection="Previous" endpoint="discover"></NavigationButton> : "N/A"}
        </Stack>
    );
};
